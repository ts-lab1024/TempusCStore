...
struct DBImpl::CompactionState {
  ...
  struct Output {
    Output() : number(0), file_size(0), time_boundary(0), time_interval(0) {}
    uint64_t number;
    uint64_t file_size;
...
Status DBImpl::RecoverLogFile(uint64_t log_number, bool last_log,
  ...
  while (reader.ReadRecord(&record, &scratch) && status.ok()) {
    ...
    status = WriteBatchInternal::InsertInto(&batch, mem); // 从日志恢复
    MaybeIgnoreError(&status);
    if (!status.ok()) {
      break;
    }
...
Status DBImpl::WriteLevel0Table(MemTable* mem, VersionEdit* edit,
  ...
  Iterator* iter = mem->NewIterator();  // memtable 迭代器
...
Status DBImpl::WriteLevel0TableTree(MemTable* mem, VersionEdit* edit,
                                Version* base) {
    mutex_.AssertHeld();
    const uint64_t start_micros = env_->NowMicros();
    // FileMetaData meta;
    ...
    Status s;
    {
        ...
        // s = BuildTable(dbname_, env_, options_, table_cache_, iter, &meta,
        // head_);
        s = BuildTablesTree(dbname_, env_, options_, table_cache_, iter, &metas,
                        versions_, &pending_outputs_, PARTITION_LENGTH, tree_head_);
        mutex_.Lock();
    }
...
void DBImpl::CompactMemTableTree() {
    mutex_.AssertHeld();
    // assert(imm_ != nullptr);
    assert(!imms_.empty());
    MemTable* imm = imms_[0];
    ...
    Version* base = versions_->current();
    base->Ref();
    Status s = WriteLevel0TableTree(imm, &edit, base);
    base->Unref();
...
Status DBImpl::TEST_CompactMemTable() {
  ...
  if (s.ok()) {
    ...
    //   s = bg_error_;
    // }
    while (has_imm_.load() && bg_error_.ok()) {
      background_work_finished_signal_.Wait();
    }
    if (has_imm_.load()) {
      s = bg_error_;
...
void DBImpl::BackgroundCall() {
  ...
  if (shutting_down_.load(std::memory_order_acquire)) {
    ...
    // No more background work after a background error.
      ...
      if (tree_head_!= nullptr) {
          BackgroundCompactionTree();
      } else {
...
void DBImpl::BackgroundCompactionTree() {
    mutex_.AssertHeld();
    ...
    if (c == nullptr) {
        ...
        CompactionState* compact = new CompactionState(c);
        status = DoCompactionWorkTree(compact);
        if (!status.ok()) {
            RecordBackgroundError(status);
        }
        CleanupCompaction(compact);
...
Status DBImpl::FinishCompactionOutputFile(CompactionState* compact,
  ...
  assert(compact != nullptr);
  TableBuilder* b = compact->builders[idx1][idx2];
  WritableFile* f = compact->outfiles[idx1][idx2];
  assert(f != nullptr);
  assert(b != nullptr);

  const uint64_t output_number = compact->outputs[idx1][idx2].number;
...
Status DBImpl::DoCompactionWork(CompactionState* compact) {
  ...
  LOG_DEBUG << "Compacting " << compact->compaction->num_input_files(0) << "@"
            ...
            << compact->compaction->num_input_files(1) << "@"
            << compact->compaction->level() + 1 << ", boundary "
            << compact->compaction->input_time_boundary();
  ...
  while (input->Valid() && !shutting_down_.load(std::memory_order_acquire)) {
    ...
    decodeKey(ikey.user_key, &current_tsid, &current_tuple_time);
    ...
    if ((current_tsid != last_tsid &&
      ...
      if (compact->builders[cur_compact_idx].back()->FileSize() >=
        ...
        compact->close_partition_file(current_tuple_time);
    ...
    if (!compact->set_smallest_key_[cur_compact_idx]) {
      compact->outputs[cur_compact_idx].back().smallest.DecodeFrom(key);
      compact->set_smallest_key_[cur_compact_idx] = true;
    }
    compact->outputs[cur_compact_idx].back().largest.DecodeFrom(key);
    compact->builders[cur_compact_idx].back()->Add(key, val);
...
Status DBImpl::DoCompactionWorkTree(CompactionState* compact) {
    const uint64_t start_micros = env_->NowMicros();
    int64_t imm_micros = 0;  // Micros spent doing imm_ compactions
    ...
    uint64_t last_sgid = std::numeric_limits<uint64_t>::max(), current_sgid;
    uint16_t last_mid = std::numeric_limits<uint64_t>::max(), current_mid;

    uint64_t current_tuple_time, prev_tuple_time;
    ...
    while (input->Valid() && !shutting_down_.load(std::memory_order_acquire)) {
        ...
        DeCodeKey(ikey.user_key.ToString(), current_sgid, current_mid, current_tuple_time);

        if ((current_sgid != last_sgid && last_sgid != std::numeric_limits<uint64_t>::max() &&
            current_mid != last_mid && last_mid != std::numeric_limits<uint64_t>::max()) ||
            (current_boundary !=
             current_tuple_time / output_interval * output_interval &&
             current_boundary != std::numeric_limits<int64_t>::max())) {
            int cur_compact_idx = compact->get_partition_idx(current_tuple_time);
...
Version* DBImpl::current() { return versions_->current(); }
