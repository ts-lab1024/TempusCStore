...
// constexpr const size_t kWritableFileBufferSize = 32 * 1024;
...
class PosixRandomAccessFile final : public RandomAccessFile {
 public:
  ...
  RandomAccessType Type() const override { return kPosix; }
...
class PosixMmapReadableFile final : public RandomAccessFile {
 public:
  ...
  size_t FileSize() const override { return length_; }
  RandomAccessType Type() const override { return kMmap; }
...
class PosixEnv : public Env {
 public:
  ...
  Status NewRandomAccessFile(const std::string& filename,
                             ...
                             const EnvOptions& opts) override {
    *result = nullptr;
    int fd = ::open(filename.c_str(), O_RDONLY | kOpenBaseFlags);
    if (fd < 0) {
      return PosixError(filename, errno);
    ...
    if (!opts.use_mmap || !mmap_limiter_.Acquire()) {
  ...
  Status NewRandomRWFile(const std::string& fname,
                         ...
                         const EnvOptions&) override {
    int fd = -1;
    int flags = cloexec_flags(O_RDWR);
    ...
    SetFD_CLOEXEC(fd);
    result->reset(new PosixRandomRWFile(fname, fd));
    return Status::OK();
  }
