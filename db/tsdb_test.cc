...
//        ASSERT_FALSE(q.second);
       // auto s = q->select({std::shared_ptr<label::MatcherInterface>("b", "1")});
        std::vector<std::shared_ptr<label::MatcherInterface>>ve;
        ve.emplace_back(std::shared_ptr<label::MatcherInterface>(new label::EqualMatcher("b", "1")));
        ve.emplace_back(std::shared_ptr<label::MatcherInterface>(new label::RegexMatcher("a", "*")));
        auto s = q->select(ve);
        ASSERT_TRUE(s);
        while (s->next()) {
          auto si = s->at();
