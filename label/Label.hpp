...
uint64_t lbs_metric_hash(const Labels &lbs, uint64_t seed = XXHASH_SEED);
uint64_t lbs_source_hash(const Labels &lbs, uint64_t seed = XXHASH_SEED);

std::unordered_map<std::string, std::string> lbs_map(const Labels &lbs);

uint64_t lb_name_label_hash(const Label lb, uint64_t seed = XXHASH_SEED);
uint64_t lb_value_label_hash(const Label lb, uint64_t seed = XXHASH_SEED);
uint64_t lbs_hash(const Labels &lbs, uint64_t seed = XXHASH_SEED);
