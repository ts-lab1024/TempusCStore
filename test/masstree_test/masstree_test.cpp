...
bool update_value(MT *mt, KeyType key, ValueType *value, bool need_delete = false) {
  KeyType key_buf{__builtin_bswap64(key)};
...
void run_insert(MT *mt) {
  ...
  while (true) {
    ...
    if (current_key > max_key)
      break;
    printf("inserting: %lu\n", current_key);
    inserted = insert_value(mt, current_key, new ValueType(current_key));
...
void run_update(MT *mt) {
  ...
  while (true) {
    ...
    if (current_key > max_key)
      break;
    auto tmp = get_value(mt, current_key);
    auto value = new ValueType (*tmp * 2);
    printf("update key: %ld to %ld\n", current_key, *value);
    updated = update_value(mt, current_key, value, true);
