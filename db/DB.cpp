...
//              request.lset().tags(i).name(), request.lset().tags(i).value()));
          matchers.emplace_back(std::shared_ptr<::tsdb::label::MatcherInterface>(new ::tsdb::label::EqualMatcher(request.lset().tags(i).name(), request.lset().tags(i).value())));
...
//        for (size_t i = 0; i < matchers.size(); i++) delete matchers[i];
  ...
  std::thread t([this]() { this->server_.listen("192.168.1.102", 9966); });
  t.detach();
}
