...
std::vector<std::shared_ptr<LogManager>> LogManager::creat_multi_LogManager(
    const int num_threads, const std::string& base_path,
    const std::string& prefix_name) {

  std::vector<std::shared_ptr<LogManager>> lms;
  for (int i = 1; i <= num_threads; ++i) {
    std::string thread_dir = base_path + "/LogManager_" + std::to_string(i);
    if (!boost::filesystem::exists(thread_dir)) {
      boost::filesystem::create_directories(thread_dir);
    }

    lms.push_back(std::make_shared<LogManager>(
        leveldb::Env::Default(), thread_dir,
        prefix_name + "_LogManager_" + std::to_string(i)));
  }
  return lms;
}
