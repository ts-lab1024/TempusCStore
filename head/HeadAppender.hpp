...
class HeadAppender : public db::AppenderInterface {
 ...
 public:
  ...
  HeadAppender(Head* head, leveldb::DB* db, int thread_id) : head(head), db(db), thread_id(thread_id) {}
  auto SetThreadId(int thread_id){
...
//    head->get_or_create_metric_id(lset);
//    head->get_or_create_source_id(lset);
    return true;
  }
...
// mock
  virtual leveldb::Status add(label::Labels&& lset, int64_t t,
                                                     double v, uint64_t &sgid,uint16_t &mid, uint64_t epoch = 0){
