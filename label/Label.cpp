...
std::string label_metric(const Labels &lbs) {
    for (auto l : lbs) {
        if (l.label == "__name__") return l.value;
    }
  return "";
}

std::string label_source_group(const Labels &lbs) {
  std::string r = "{";
  for (int i = 0; i < lbs.size(); i++) {
      if (lbs[i].label == "__name__") continue;
      r += lbs[i].label + "+" + lbs[i].value;
  }
  r += "}";
  return r;
...
uint64_t lb_name_label_hash(const Label lb, uint64_t seed){
    std::string input = lb.label;
    return ::XXH64(input.c_str(),input.size(),seed);
}
uint64_t lb_value_label_hash(const Label lb, uint64_t seed){
    std::string input = lb.value;
    return ::XXH64(input.c_str(),input.size(),seed);
}

uint64_t lbs_hash(const Labels &lbs, uint64_t seed) {
  std::string input;
...
uint64_t lbs_metric_hash(const Labels &lbs, uint64_t seed) {
  std::string input;
  for (const Label &l : lbs) {
    if (l.label == "__name__") {
      input += l.value;
...
uint64_t lbs_source_hash(const Labels &lbs, uint64_t seed) {
  std::string input;
  for (const Label &l : lbs) {
    if (l.label == "__name__") {
      continue;
...
uint64_t lbs_hash_group(const std::vector<Labels> &labels) {
  std::string input;
