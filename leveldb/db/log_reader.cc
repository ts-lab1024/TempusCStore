...
bool series(const Slice& s, std::vector<RefSeries>* v) {
  ...
  while (i < s.size()) {
    ...
    i += 8;
    v->back().flushed_txn = DecodeFixed64BE(s.data() + i);
    i += 8;
    v->back().log_clean_txn = DecodeFixed64BE(s.data() + i);
    i += 8;
...
bool series(const Slice& s, RefSeries* rs) {
  if (s.data()[0] != kSeries) return false;
  int i = 1;
  uint32_t size;
  Slice result;
  while (i < s.size()) {
    rs->ref = DecodeFixed64BE(s.data() + i);
    i += 8;
    rs->flushed_txn = DecodeFixed64BE(s.data() + i);
    i += 8;
    rs->log_clean_txn = DecodeFixed64BE(s.data() + i);
    i += 8;
    Slice tmps(s.data() + i, s.size() - i);
    GetVarint32(&tmps, &size);
    i = s.size() - tmps.size();
    ...
    for (uint32_t j = 0; j < size; j++) {
      ...
      std::string value;
      value.append(result.data(), result.size());
      rs->lset.emplace_back(label, value);
    }
  }
  return true;
}
...
bool samples(const Slice& s, std::vector<RefSample>* v) {
  ...
  v->back().ref = DecodeFixed64BE(s.data() + i);
  i += 8;
  v->back().logical_id = DecodeFixed64BE(s.data() + i);
  i += 8;
  ...
  while (i < s.size()) {
    ...
    GetVarint64(&tmps, &tmp);
    v->back().logical_id =
        (uint64_t)((int64_t)(v->at(v->size() - 2).logical_id) +
                   ::tsdb::base::decode_signed_varint(tmp));
    i = s.size() - tmps.size();
...
bool flush(const Slice& s, RefFlush* f) {
  ...
  f->logical_id = DecodeFixed64BE(s.data() + 9);
  f->txn = DecodeFixed64BE(s.data() + 17);
  return true;
}
...
bool treeSeries(const Slice& s, std::vector<tsdb::tsdbutil::TreeRefSeries>* v) {
    ...
    while (i < s.size()) {
        v->emplace_back();
        v->back().mid = DecodeFixed16(s.data() + i);
        i += 2;
        v->back().sgid = DecodeFixed64BE(s.data() + i);
        i += 8;
        v->back().flushed_txn = DecodeFixed64BE(s.data() + i);
        i += 8;
        v->back().log_clean_txn = DecodeFixed64BE(s.data() + i);
...
bool treeSeries(const Slice& s, tsdb::tsdbutil::TreeRefSeries* rs) {
    ...
    Slice result;
    while (i < s.size()) {
        rs->mid = DecodeFixed16(s.data() + i);
        i += 2;
        rs->sgid = DecodeFixed64BE(s.data() + i);
        i += 8;
        rs->flushed_txn = DecodeFixed64BE(s.data() + i);
        i += 8;
        rs->log_clean_txn = DecodeFixed64BE(s.data() + i);
...
bool treeSamples(const Slice& s, std::vector<tsdb::tsdbutil::TreeRefSample>* v) {
    if (s.data()[0] != kSample) return false;
    ...
    while (i < s.size()) {
        ...
        Slice tmps(s.data() + i, s.size() - i);
        GetVarint64(&tmps, &tmp);
        v->back().mid = (uint64_t)((int64_t)(v->at(v->size() - 2).mid) +
                                   ::tsdb::base::decode_signed_varint(tmp));
        i = s.size() - tmps.size();
        ...
        GetVarint64(&tmps, &tmp);
        v->back().sgid =
                (uint64_t)((int64_t)(v->at(v->size() - 2).sgid) +
                           ::tsdb::base::decode_signed_varint(tmp));
        i = s.size() - tmps.size();
...
bool treeFlush(const Slice& s, tsdb::tsdbutil::TreeRefFlush* f) {
    if (s.data()[0] != kFlush) return false;

    f->mid = DecodeFixed16(s.data() + 1);
    f->sgid = DecodeFixed64BE(s.data() + 3);
    f->txn = DecodeFixed64BE(s.data() + 11);
}
...
RandomReader::RandomReader(RandomRWFile* file)
    : file_(file), backing_store_(new char[kBlockSize]), buffer_() {}

RandomReader::~RandomReader() { delete[] backing_store_; }

bool RandomReader::ReadRecord(uint64_t pos, Slice* record,
                              std::string* scratch) {
  current_offset_ = pos;
  if (kBlockSize - current_offset_ % kBlockSize < kHeaderSize)
    current_offset_ = kBlockSize * (current_offset_ / kBlockSize + 1);
    ...
    switch (record_type) {
      case kFullType:
        if (in_fragmented_record) {
          ...
          // at the beginning of the next block.
          if (!scratch->empty()) {
            // ReportCorruption(scratch->size(), "partial record without
            // end(1)");
          }
        }
        scratch->clear();
...
unsigned int RandomReader::ReadPhysicalRecord(Slice* result) {
  while (true) {
    Status status =
        file_->Read(current_offset_, kHeaderSize, &buffer_, backing_store_);
    if (!status.ok()) {
    ...
    status = file_->Read(current_offset_ + kHeaderSize, length, &buffer_,
                         backing_store_);
    if (!status.ok()) {
      // printf("s2:%s\n", status.ToString().c_str());
      buffer_.clear();
