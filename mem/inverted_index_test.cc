...
TEST_F(InvertedIndexTest, Test1_2) {
  ...
  for (int i = 0; i < 10; i++)
    matchers.push_back(std::shared_ptr<label::MatcherInterface>(new label::EqualMatcher(
        "key" + std::to_string(i + 1), "value" + std::to_string(i + 1))));
  {
    std::promise<bool> p;
...
TEST_F(InvertedIndexTest, Test3_2) {
  ...
  std::vector<std::vector<uint64_t>> lists(1);
  std::vector<std::shared_ptr<label::MatcherInterface>> matchers(
      {std::shared_ptr<label::MatcherInterface>(new label::EqualMatcher("label0", "label0_0"))});
  {
    std::promise<bool> p;
    std::future<bool> f = p.get_future();
    tdb.async_select(
 ...
 // delete matchers[0];
  matchers.clear();
  matchers.push_back(std::shared_ptr<label::MatcherInterface>(new label::EqualMatcher("label9", "label9_5000")));
  matchers.push_back(std::shared_ptr<label::MatcherInterface>(new label::EqualMatcher("label0", "label0_223")));
  {
    std::promise<bool> p;
    std::future<bool> f = p.get_future();
    tdb.async_select(
...
//  delete matchers[1];
  ...
  for (int i = 0; i < num_ts; i++) {
    matchers.push_back(
        std::shared_ptr<label::MatcherInterface>(new label::EqualMatcher("label5", "label5_" + std::to_string(i))));
    {
      std::promise<bool> p;
      std::future<bool> f = p.get_future();
      tdb.async_select(
