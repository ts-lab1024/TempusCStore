...
protected:
    ...
    alignas(ATOMIC_CACHE_LINE_SIZE) std::atomic<unsigned> head_ = {};
    alignas(ATOMIC_CACHE_LINE_SIZE) std::atomic<unsigned> tail_ = {};
...
class AtomicQueue : public AtomicQueueCommon<AtomicQueue<T, SIZE, NIL, MINIMIZE_CONTENTION, MAXIMIZE_THROUGHPUT, TOTAL_ORDER, SPSC>> {
    ...
    static constexpr int SHUFFLE_BITS = details::GetIndexShuffleBits<MINIMIZE_CONTENTION, size_, ATOMIC_CACHE_LINE_SIZE / sizeof(std::atomic<T>)>::value;
    ...
    alignas(ATOMIC_CACHE_LINE_SIZE) std::atomic<T> elements_[size_];
...
class AtomicQueue2 : public AtomicQueueCommon<AtomicQueue2<T, SIZE, MINIMIZE_CONTENTION, MAXIMIZE_THROUGHPUT, TOTAL_ORDER, SPSC>> {
    ...
    static constexpr unsigned size_ = MINIMIZE_CONTENTION ? details::round_up_to_power_of_2(SIZE) : SIZE;
    static constexpr int SHUFFLE_BITS = details::GetIndexShuffleBits<MINIMIZE_CONTENTION, size_, ATOMIC_CACHE_LINE_SIZE / sizeof(State)>::value;
    static constexpr bool total_order_ = TOTAL_ORDER;
    static constexpr bool spsc_ = SPSC;
    static constexpr bool maximize_throughput_ = MAXIMIZE_THROUGHPUT;

    alignas(ATOMIC_CACHE_LINE_SIZE) std::atomic<unsigned char> states_[size_] = {};
    alignas(ATOMIC_CACHE_LINE_SIZE) T elements_[size_] = {};
...
class AtomicQueueB : private std::allocator_traits<A>::template rebind_alloc<std::atomic<T>>,
    ...
    static constexpr auto ELEMENTS_PER_CACHE_LINE = ATOMIC_CACHE_LINE_SIZE / sizeof(std::atomic<T>);
    ...
    alignas(ATOMIC_CACHE_LINE_SIZE) unsigned size_;
...
class AtomicQueueB2 : private std::allocator_traits<A>::template rebind_alloc<unsigned char>,
    ...
    static constexpr auto STATES_PER_CACHE_LINE = ATOMIC_CACHE_LINE_SIZE / sizeof(AtomicState);
