...
/********************* intersectPostingsLists **************************/
    ...
    class TreeIntersectPostingsLists : public ::tsdb::index::PostingsInterface {
    private:
        std::vector<std::unique_ptr<index::PostingsInterface>> p;

    public:
        TreeIntersectPostingsLists(std::vector<std::unique_ptr<index::PostingsInterface>>&& _p) : p(std::move(_p)) {}

        ~TreeIntersectPostingsLists() {}

        bool recursive_next(uint64_t max) const {
            while (true) {
                bool find = true;
                for (size_t i = 0; i < p.size(); i++) {
                    bool has_next = p[i]->seek(max);
                    if (!has_next) return false;
                    if (p[i]->at() > max) {
                        max = p[i]->at();
                        find = false;
        ...
        bool next() const {
            uint64_t max = 0;
            for (size_t i = 0; i < p.size(); i++) {
                if (!p[i]->next()) return false;
                if (p[i]->at() > max) max = p[i]->at();
            }
            return recursive_next(max);
        }
        ...
        bool seek(uint64_t v) const {
            ...
            for (size_t i = 0; i < p.size(); i++) {
                if (!p[i]->seek(v)) return false;
                if (p[i]->at() > max) max = p[i]->at();
            }
            return recursive_next(max);
        }

        uint64_t at() const { return p[0]->at(); }
    };
