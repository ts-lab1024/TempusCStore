...
void Version::AddIterators(const ReadOptions& options, int level, int start_idx,
  ...
  for (size_t i = start_idx; i < files_[level].size(); i++) {
    if (files_[level][i]->time_boundary != tb ||
      ...
      return;
    decodeKey(files_[level][i]->smallest.user_key(), &min_tsid, nullptr);
    decodeKey(files_[level][i]->largest.user_key(), &max_tsid, nullptr);
    if (min_tsid <= tsid && tsid <= max_tsid)   //todo
      iters->push_back(vset_->table_cache_->NewIterator(
          options, files_[level][i]->number, files_[level][i]->file_size));
  }
}

void Version::AddIterators(const ReadOptions &options, int level, int start_idx, std::vector<Iterator *> *iters, uint64_t sgid, uint16_t mid) {
    int64_t tb = files_[level][start_idx]->time_boundary,
            ti = files_[level][start_idx]->time_interval;
    uint64_t min_sgid, max_sgid;
    uint16_t min_mid, max_mid;
    uint64_t mint, maxt;

    for (size_t i = start_idx; i < files_[level].size(); i++) {
        if (files_[level][i]->time_boundary != tb || files_[level][i]->time_interval != ti)
            return;
        DeCodeKey(files_[level][i]->smallest.user_key().ToString(), min_sgid, min_mid, mint);
        DeCodeKey(files_[level][i]->largest.user_key().ToString(), max_sgid, max_mid, maxt);
        if (min_sgid <= sgid && sgid <= max_sgid && min_mid <= mid && mid <= max_mid) {
            iters->push_back(vset_->table_cache_->NewIterator(options, files_[level][i]->number, files_[level][i]->file_size));
        }
    }
}
 ...
 public:
  ...
  void MaybeAddFile(Version* v, int level, FileMetaData* f) {
    if (levels_[level].deleted_files.count(f->number) > 0) {
      ...
      if (level > 0 && !files->empty()) {
        // Must not overlap
        assert((*files)[files->size() - 1]->time_boundary < f->time_boundary ||
...
void VersionSet::Finalize(Version* v) {
  ...
  if (v->time_partition_indexes_[0].size() > L0_MERGE_FACTOR)
...
Iterator* VersionSet::MakeInputIterator(Compaction* c) {
  ...
  int space = c->inputs_[0].size() + 1 + c->inputs_[1].size();
  Iterator** list = new Iterator*[space];
...
Compaction* VersionSet::PickCompaction() {
  ...
  if (current_->time_partition_indexes_[0].size() > L0_MERGE_FACTOR) {
    ...
    for (size_t i = 0; i < current_->time_partition_indexes_[0][1]; i++) {
    ...
    // Note(Alec): will not happen in current system.
