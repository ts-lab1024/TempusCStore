...
Status BuildTablesTree(const std::string& dbname, Env* env, const Options& options,
                   TableCache* table_cache, Iterator* iter,
                   std::vector<FileMetaData>* metas, VersionSet* const vset,
                   std::set<uint64_t>* pending_outputs,
                   int64_t partition_length, ::tsdb::head::TreeHead* head) {
    Status s;
    iter->SeekToFirst();
    ...
    uint64_t key_time;
    int64_t time, time_boundary;
    std::vector<tsdb::tsdbutil::TreeRefFlush> flush_marks;
    ...
    if (iter->Valid()) {
        ...
        for (; iter->Valid(); iter->Next()) {
            ...
            user_key = ExtractUserKey(key);
            DeCodeKey(user_key.ToString(), sgid, mid, key_time);
            time_boundary = key_time / partition_length * partition_length;

            if (head && !options.use_log) {
                uint16_t mid = leveldb::DecodeFixed16(iter->value().data() + 1);
                uint64_t sgid = leveldb::DecodeFixed64BE(iter->value().data() + 3);
                flush_marks.emplace_back(mid, sgid, txn);
            }
            ...
            if (iter->value().size() >= 4096) {
                std::string v;
                v.append(iter->value().data(), iter->value().size());
                builders[time_boundary]->Add(key, v);
            } else {
                memcpy(buf, iter->value().data(), iter->value().size());
                builders[time_boundary]->Add(key, Slice(buf, iter->value().size()));
            }
