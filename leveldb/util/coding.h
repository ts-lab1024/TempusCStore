...
void PutFixed32BE(std::string* dst, uint32_t value);
...
bool GetFixed64Ptr(const char* input, uint64_t* value);
bool GetFixed32Ptr(const char* input, uint32_t* value);
bool GetFixed16Ptr(const char* input, uint16_t* value);
...
inline void EncodeFixed16(char* buf, uint32_t value) {
  buf[0] = value & 0xff;
  buf[1] = (value >> 8) & 0xff;
}
...
inline void EncodeFixed32BE(char* dst, uint32_t value) {
  uint8_t* const buffer = reinterpret_cast<uint8_t*>(dst);
...
void encodeKey(std::string* str, uint64_t tsid, int64_t t);
void decodeKey(const Slice& s, uint64_t* tsid, int64_t* t);
...
//void encodeKey(std::string * str, uint64_t sgid, uint64_t mid, int64_t t);
//
//    void encodeKey(std::string * str, uint64_t sgid, uint64_t mid, int64_t t);
