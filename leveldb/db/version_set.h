...
class Version {
 public:
  ...
  void AddIterators(const ReadOptions&, int level, int start_idx, std::vector<Iterator*>* iters, uint64_t sgid, uint16_t mid);
...
class Compaction {
 public:
  ...
  int64_t input_time_boundary() const { return inputs_[0][0]->time_boundary; }

  std::vector<int64_t> input_time_boundaries() const {
    return input_time_boundaries_;
  }
  int64_t time_interval() const { return output_time_intervals_[0]; }
