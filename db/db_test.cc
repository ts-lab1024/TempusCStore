  ...
  void queryDevOps1(leveldb::DB* db, int64_t endtime,
    ...
    if (query_types["1-1-1"]) {
      ...
      for (int round = 0; round < iteration; round++) {
        ...
        std::vector<std::shared_ptr<tsdb::label::MatcherInterface>>matchers(
            {std::shared_ptr<label::MatcherInterface>(new label::EqualMatcher(matchers1[round % 50])), std::shared_ptr<label::MatcherInterface>(new label::EqualMatcher(matchers2[0]))});
    ...
    if (query_types["5-1-1"]) {
      ...
      for (int round = 0; round < iteration; round++) {
        ...
        for (int j = 0; j < 5; j++) {
          std::vector<std::shared_ptr<tsdb::label::MatcherInterface>>matchers(
              {std::shared_ptr<label::MatcherInterface>(new label::EqualMatcher(matchers1[round % 50])), std::shared_ptr<label::MatcherInterface>(new label::EqualMatcher(matchers2[j]))});
          std::unique_ptr<::tsdb::querier::SeriesSetInterface> ss =
              q.select(matchers);
          while (ss->next()) {
            std::unique_ptr<::tsdb::querier::SeriesInterface> series = ss->at();
...
//                {&matchers1[count % 50], &matchers2[j]});
            std::vector<std::shared_ptr<tsdb::label::MatcherInterface>>matchers(
                {std::shared_ptr<label::MatcherInterface>(new label::EqualMatcher(matchers1[count % 50])), std::shared_ptr<label::MatcherInterface>(new label::EqualMatcher(matchers2[j]))});
            std::unique_ptr<::tsdb::querier::SeriesSetInterface> ss =
                q.select(matchers);
            while (ss->next()) {
