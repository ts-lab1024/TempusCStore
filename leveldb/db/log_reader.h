...
bool series(const Slice& s, std::vector<RefSeries>* series);
bool series(const Slice& s, RefSeries* series);
...
// │                              . . .                                       │
// └──────────────────────────────────────────────────────────────────────────┘
bool samples(const Slice& s, std::vector<RefSample>* samples);
...
class RandomReader {
 public:
  RandomReader(RandomRWFile* file);
  ...
  // will only be valid until the next mutating operation on this
  // reader or the next mutation to *scratch.
  bool ReadRecord(uint64_t pos, Slice* record, std::string* scratch);
